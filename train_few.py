...
from utils import augment, _freeze_stages, Logger,normalize,setup_seed
import json
import warnings
...
def main():
    ...
    for epoch in range(args.epoch):
        ...
        loss_list = []
        for (image, mask, label) in tqdm(train_loader):
        ...
        if score >= save_score:
            save_score = score
            ckp_path = f'./{args.save_dir}/{args.obj}_{args.shot}.pth'
            ...
            logger.info(f'best epoch found: epoch {epoch}')
